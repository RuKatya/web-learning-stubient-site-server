const { SERVER_ERROR_MESSAGE, UNKNOWN_ERROR_MESSAGE } = require('../../utils/errorMessages');
  ...
  try {
    ...
    db.query(query, (err, subjects) => {
      if (err) {
        ...
        return res.status(httpCodes.SERVER_ERROR).send({ continueWork: false, message: UNKNOWN_ERROR_MESSAGE });
      }
    ...
    console.error(error);
    return res.status(httpCodes.SERVER_ERROR).send({ message: SERVER_ERROR_MESSAGE });
  }
};
  ...
  try {
    ...
    db.query(query, (err, result) => {
      if (err) {
        ...
        return res.status(httpCodes.REQUEST_CONFLICT).send({ continueWork: false, message: UNKNOWN_ERROR_MESSAGE });
      }
...
exports.removeSubject = async (req, res) => {
  try {
    console.log(req.cookies.myCookieName);
    ...
    db.query(deleteQuery, (err, result) => {
      if (err) {
        ...
        return res.status(httpCodes.BAD_REQUEST).send({ continueWork: false, message: UNKNOWN_ERROR_MESSAGE });
      }
      ...
      db.query(deleteAllTitles, (err, deletedQuestions) => {
        ...
        db.query(deleteAllQuestions, (err, deletedQuestions) => {
          ...
          return res.status(httpCodes.OK).send({ continueWork: true, id, message: 'Subject Deleted' });
        });
      });
    });
  } catch (error) {
...
exports.updateSubject = async (req, res) => {
  try {
    ...
    db.query(updateQuery, (err, result) => {
      ...
      return res
        .status(httpCodes.OK)
        .send({ continueWork: true, id, SubjectName: newName, message: 'Subject Updated' });
    });
  } catch (error) {
